    ...
    <Layout>
      <div className="px-4 pt-safe">
        <div className="pt-4 pb-6">
          <h1 className="text-xl font-semibold text-text-main">Today's Overview</h1>
          <p className="text-warm-gray text-sm mt-0.5">{format(now, 'EEEE, d MMMM')}</p>
        </div>

        {todayLogs.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <span className="text-5xl mb-4">🛋️</span>
            <p className="text-warm-gray text-sm">Nothing logged yet today.</p>
            <p className="text-warm-gray text-xs mt-1">Tap + to log your first chore!</p>
          </div>
        ) : (
          <div className="space-y-6">
            {byUser.map(({ profile, logs: userLogs, score }) => {
              ...
              if (!userLogs.length) return (
                <div key={profile.id} className="bg-white rounded-3xl p-4 shadow-card">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="w-3 h-3 rounded-full" style={{ backgroundColor: color.hex }} />
                    <span className="font-medium text-text-main">{profile.display_name}</span>
                    <span className="text-warm-gray text-sm ml-auto">0 pts</span>
                  </div>
                  <p className="text-xs text-warm-gray pl-5">Nothing yet today</p>
                </div>
              )
              return (
                <div key={profile.id} className="bg-white rounded-3xl p-4 shadow-card">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <span className="w-3 h-3 rounded-full" style={{ backgroundColor: color.hex }} />
                      <span className="font-semibold text-text-main">{profile.display_name}</span>
                    </div>
                      ...
                      return (
                        <div key={log.id} className="flex items-center justify-between py-1">
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <span className="text-sm text-text-main">{chore?.name ?? '—'}</span>
                          </div>
                          <div className="flex items-center gap-3 text-xs text-warm-gray tabular-nums">
            ...
            {byUser.length === 2 && (
              <div className="bg-white rounded-3xl p-4 shadow-card">
                <h3 className="text-sm font-semibold text-text-main mb-3">Today's tally</h3>
                  ...
                  {byUser.map(({ profile, score }, i) => {
                    ...
                    return (
                      <div key={profile.id} className={`flex-1 ${i === 1 ? 'text-right' : ''}`}>
                        <span className="text-xs text-warm-gray">{profile.display_name}</span>
                        <p className="text-lg font-bold tabular-nums" style={{ color: color.hex }}>
                          {score}
                        </p>
                        <p className="text-xs text-warm-gray">{pct}%</p>
                      </div>
                    )
                  })}
                </div>
